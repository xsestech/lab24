...
token_t list_iter_val(list_iter_t iter) {
...
void list_insert_before(list_iter_t iter, token_t val) {
...
void list_insert_after(list_iter_t iter, token_t val) {
  list_handle_t list = iter.list;
  if (list->container[list->top_element_empty].next == -1) {
    list_grow(iter.list);
  }
...
token_t list_remove(list_iter_t iter) {
  list_handle_t list = iter.list;
...
void list_push_back(list_handle_t list, token_t val) {
  list_insert_after(list_end(list), val);
}
void list_push_front(list_handle_t list, token_t val) {
  list_insert_before(list_begin(list), val);
}
